...
const About = () => {
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '64px 40px' }}>
      ...
      <div className="card" style={{ padding: '36px', marginBottom: 28, background: '#f8fafc', border: '1px solid #a7f3d0' }}>
        <h2 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 24, fontWeight: 700, marginBottom: 14 }}>Our Mission</h2>
      ...
      <div className="grid-2" style={{ gap: 22, marginBottom: 36 }}>
        {[
          [<User size={30} key="students" />, 'For Students', 'Build a verified skills portfolio on Credify. Complete real projects and get verified certificates that employers can verify by unique ID.'],
          ...
          <div key={title} className="card" style={{ padding: '26px' }}>
            <div style={{ fontSize: 30, marginBottom: 10, color: '#059669' }}>{icon}</div>
            <h3 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 16, fontWeight: 700, marginBottom: 8, color: '#0d1f35' }}>{title}</h3>
            <p style={{ fontSize: 14, color: '#4a6080', lineHeight: 1.65 }}>{desc}</p>
          </div>
        ))}
      </div>

      <div style={{ background: '#022c22', borderRadius: 12, padding: '44px', textAlign: 'center' }}>
        <h2 style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 28, fontWeight: 700, color: '#fff', marginBottom: 12 }}>
          Join Credify Today
        </h2>
